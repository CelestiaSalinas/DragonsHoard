...
public class BuddingEarthCrystalBlock extends CrystalBlock {
    ...
    public BuddingEarthCrystalBlock(Properties p_152726_) {
        super(p_152726_);
    }
    ...
    public void randomTick(BlockState p_152728_, ServerWorld p_152729_, BlockPos p_152730_, Random p_152731_) {
        if (p_152731_.nextInt(5) == 0) {
            ...
            if (canClusterGrowAtState(blockstate)) {
                block = DHBlocks.SMALL_EARTH_CLUSTER.get();
            } else if (blockstate.is(DHBlocks.SMALL_EARTH_CLUSTER.get()) && blockstate.getValue(FireCrystalClusterBlock.FACING) == direction) {
                block = DHBlocks.MEDIUM_EARTH_CLUSTER.get();
            } else if (blockstate.is(DHBlocks.MEDIUM_EARTH_CLUSTER.get()) && blockstate.getValue(FireCrystalClusterBlock.FACING) == direction) {
                block = DHBlocks.LARGE_EARTH_CLUSTER.get();
            } else if (blockstate.is(DHBlocks.LARGE_EARTH_CLUSTER.get()) && blockstate.getValue(FireCrystalClusterBlock.FACING) == direction) {
                block = DHBlocks.CHARGED_EARTH_CRYSTAL.get();
            }
