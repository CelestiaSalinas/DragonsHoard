...
public class EarthCrystalClusterBlock extends CrystalBlock implements IWaterLoggable {
    ...
    public EarthCrystalClusterBlock(int p_152015_, int p_152016_, Properties p_152017_) {
        super(p_152017_);
    ...
    @OnlyIn(Dist.CLIENT)
    public void animateTick(BlockState p_180655_1_, World worldIn, BlockPos p_180655_3_, Random rand) {
        Direction direction = p_180655_1_.getValue(FACING);
        double d0 = (double)p_180655_3_.getX() + 0.55D - (double)(rand.nextFloat() * 0.1F);
        double d1 = (double)p_180655_3_.getY() + 0.55D - (double)(rand.nextFloat() * 0.1F);
        double d2 = (double)p_180655_3_.getZ() + 0.55D - (double)(rand.nextFloat() * 0.1F);
        double d3 = (double)(0.4F - (rand.nextFloat() + rand.nextFloat()) * 0.4F);
        if (rand.nextInt(5) == 0) {
            worldIn.addParticle(ParticleRegistry.ORANGE_GLITTER.get(), d0 + (double)direction.getStepX() * d3, d1 + (double)direction.getStepY() * d3, d2 + (double)direction.getStepZ() * d3, rand.nextGaussian() * 0.005D, rand.nextGaussian() * 0.005D, rand.nextGaussian() * 0.005D);
        }
        if (rand.nextInt(5) == 0) {
            worldIn.addParticle(ParticleTypes.AMBIENT_ENTITY_EFFECT, d0 + (double)direction.getStepX() * d3, d1 + (double)direction.getStepY() * d3, d2 + (double)direction.getStepZ() * d3, rand.nextGaussian() * 0.005D, rand.nextGaussian() * 0.005D, rand.nextGaussian() * 0.005D);
        }
